...
public class OperacionController {
    ...
    public float getSaldoBilletera(@Parameter(description = "billeteraId que desea consultar. Use 1 al 17 para probar. ", required = true, example = "12") @PathVariable("billeteraId") Long billeteraId) {
    ...
    public float getSaldoUsuario(@Parameter(description = "userId que desea consultar. Use 1 al 10 para probar. ", required = true, example = "3") @PathVariable("userId") Long userId) {
    ...
    @ApiResponses(value = { @ApiResponse(description = "successful operation", content = {
            @Content(mediaType = "application/json", examples = {@ExampleObject(value = "{\"bDestino\": 2,\"divOrigen\": \"peso\", \"divDestino\": \"peso\",\"monto\": 345}")}) }) })//schema = @Schema(implementation = Operacion.class)) }) })
